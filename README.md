...
> ## ⚠️ 重要：删除插件前必须先备份数据！
>
> **本插件所有数据（图片、标签、笔记）仅存储在你的浏览器本地（IndexedDB）。**
>
> 以下任意操作都会导致数据永久丢失，且无法恢复：
> - 在 `chrome://extensions/` 中卸载/删除本插件
> - 清除浏览器缓存或站点数据
> - 换电脑或重装浏览器
>
> **操作前请务必：点击插件页右上角的 🗄️（数据备份）按钮 → 「导出全部数据」，将 JSON 备份文件保存到本地！**
>
> 换设备或重装后，通过同一按钮「导入备份」即可恢复所有数据。
...
- **数据备份迁移**：支持一键导出全部数据为 JSON 备份文件，换设备或重装时可直接导入恢复
