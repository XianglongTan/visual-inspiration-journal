/**
 * 配置模板。首次运行时会复制为 config.ts；
 * 请在 config.ts 中填入你的 API Key。
 */
export const APP_CONFIG = {
  CEREBRAS: {
    ...
    PROMPT:
      '请基于这张截图的具体使用场景和界面语境，从专业UI/视觉设计角度，提炼5-10个最关键的设计关键词。关键解析设计中的布局结构、组件形态、字体、颜色、材质、层级关系、交互定义，避免抽象评价，需直接可用于复刻或检索类似设计。用list输出(["关键词1"...])，不要输出其他。必须用中文回复。',
    TEMPERATURE: 1.0,
    TOP_P: 1.0,
    MAX_TOKENS: 16384,
    STREAM: false,
